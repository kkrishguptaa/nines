...
class AnthropicSynthesizer:
    ...
    def __call__(self, task: Task, **kwargs) -> VerifierMeta | None:
        ...
        prompt = (
            ...
            "Rules:\n"
            "- Prefer exec + example calls; on failure raise AssertionError('short reason') "
            "or assert condition, 'short reason' — so failures are attributable\n"
            "- Call `_nines_strip_fences(output)` before exec (helper is pre-defined)\n"
            "- Do NOT write markdown fence characters in your source\n"
            "- MUST reject empty output, syntax errors, missing function, "
